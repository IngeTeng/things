...
if(!empty($_GET['add_role'])){
    $s_add_role  = safeCheck($_GET['add_role'],0);
}else{
    $s_add_role  = '';
}
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#searchproduct').click(function(){
                ...
                s_add_role      = $('#search_add_role').val();
                s_cateid        = $('#search_cateid').val();
                s_title         = $('#search_title').val();
                location.href='product_list.php?add_role='+s_add_role+'&cateid='+s_cateid+'&title='+s_title;
            });
...
<div id="container">
    <?php include('shop_menu.inc.php');?>
        <div id="maincontent">
            <div id="position">当前位置：<a href="partner_list.php">分销商管理</a> > 分销商设置</div>
            <div id="handlelist">
                <?php
                //初始化
                $totalcount= Product::search(0, 0, $s_cateid, $s_add_role,$s_title, 1);
                $shownum   = 5;
                $pagecount = ceil($totalcount / $shownum);
                $page      = getPage($pagecount);//点击页码之后在这函数里面获取页码
                $rows      = Product::search($page, $shownum, $s_cateid, $s_add_role,$s_title);
                ?>
              ...
              <!--   <input class="order-input" placeholder="产品分类"  name="search_cateid" id="search_cateid" value="<?php echo $s_cateid?>" style="width:15%;height:16px;" type="text"> -->
                ...
                <input class="order-input" placeholder="添加者姓名"  name="search_add_role" id="search_add_role" value="<?php echo $s_add_role?>" style="width:15%;height:16px;" type="text">
                    ...
                    foreach($rows as $row){
                       ...
                       // $isnew          = Product::getNewName($row['isnew']);
                        ...
                        echo '<tr>          
                                            ...
                                            <td class="center">'.$add_role_cate.'<br>'.$row['add_role'].'</td>
                                            <td class="center">'.$createtime.'</td> 
                                                        
                                            <td class="center">                            <a class="see" href="comment_list.php?productid='.$row['id'].'"><img src="images/dot_see.png"/></a>     

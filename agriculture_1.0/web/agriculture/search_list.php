...
if(!empty($_GET['cate'])){
    $s_cate  = safeCheck($_GET['cate']);
    $product_cate = table_product::getInfoByCate($s_cate);
}
?>
<!doctype html>
<html>
...
<div class="container">
    ...
    <div class="am-cf am-g">
        ...
        <div class="am-panel am-panel-default am-panel-warning mg10">
            <div class="am-panel-hd"><a href="#" class="am-text-warning">全部产品</a></div>
            <div class="am-panel-bd">
                <ul data-am-widget="gallery" class="am-gallery am-avg-sm-3 am-avg-md-3 am-avg-lg-4 am-gallery-default" >
                    <?php 
                    if(empty($s_cate)){
                    $rows = table_product::getInfoByTitle($s_title);
                    //var_dump($rows);
                     if(!empty($rows)) {
                        foreach($rows as $row) {
                    ...
                    echo '<li>
                            <div class="am-gallery-item">
                                <a href="https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$appid.'&redirect_uri='.urlencode("http://www.yanxin325.com/agriculture_1.0/web/agriculture/product_detail.php").'?productid='.$product['id'].'&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect" class="good-item">
                                    <img src="'.$pic.'"  alt="农贸复合肥"/>
                                    <h3 class="am-gallery-title">'.$product['title'].'</h3>
                                <div class="am-gallery-desc red">￥'.$product['sale_price'].'</div>
                                </a>
                            </div>
                        </li>';
                        }
